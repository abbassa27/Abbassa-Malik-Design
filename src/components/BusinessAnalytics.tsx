...
function GrowthBadge({ pct }: { pct: number }) {
  ...
  const color = flat ? "text-muted bg-cream" : up ? "text-green-700 bg-green-50" : "text-red-700 bg-red-50";
  return (
...
function BarRow({ label, value, maxVal, color, sub }: {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs text-muted w-10 flex-shrink-0 text-right">{label}</span>
      ...
      <span className="text-xs font-bold text-charcoal w-14 flex-shrink-0">{fmt(value)}</span>
      {sub && <span className="text-xs text-muted w-8 flex-shrink-0">{sub}</span>}
    </div>
  );
}
...
export default function BusinessAnalytics() {
  ...
  if (!data) return <p className="text-muted text-center py-12">Failed to load business analytics.</p>;
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-serif text-2xl font-bold text-charcoal">Business Analytics</h2>
          <p className="text-muted text-sm mt-0.5">Profitability, growth & service insights</p>
        </div>
        <div className="flex items-center gap-2">
          <select
            value={year}
            onChange={(e) => setYear(Number(e.target.value))}
            className="px-3 py-2 rounded-full border border-gold-light text-sm text-charcoal bg-white focus:outline-none focus:ring-2 focus:ring-gold/30"
          >
          ...
          <button
            onClick={fetchData}
            className="flex items-center gap-2 text-sm text-muted hover:text-charcoal border border-gold-light px-3 py-2 rounded-full hover:border-gold transition-colors"
          >
      ...
      <div className="bg-white rounded-2xl border border-gold-light/50 p-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-charcoal">Revenue Growth</h3>
          </div>
            ...
            {(["monthly","quarterly","yearly"] as const).map((v) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all capitalize ${
                  view === v ? "bg-white shadow-sm text-charcoal" : "text-muted hover:text-charcoal"
                }`}
              >
                {v}
              </button>
        ...
        {/* Summary row */}
        <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-cream">
          <div>
            <p className="text-xs text-muted">Total {view === "yearly" ? "All-Time" : year}</p>
          ...
          <div>
            <p className="text-xs text-muted">Peak {view === "monthly" ? "Month" : view === "quarterly" ? "Quarter" : "Year"}</p>
            <p className="text-charcoal font-bold text-sm">
      ...
      <div className="bg-white rounded-2xl border border-gold-light/50 p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h3 className="font-semibold text-charcoal">Top Services — Breakdown</h3>
        </div>

        {services.length === 0 ? (
          <p className="text-muted text-sm">No service data yet.</p>
        ) : (
          <>
            {/* Desktop Table */}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-cream">
                    <th className="text-left text-xs font-semibold text-muted uppercase tracking-wide py-2 pr-4">Service / Package</th>
                    <th className="text-right text-xs font-semibold text-muted uppercase tracking-wide py-2 px-4">Orders</th>
                    <th className="text-right text-xs font-semibold text-muted uppercase tracking-wide py-2 px-4">Total Revenue</th>
                    <th className="text-right text-xs font-semibold text-muted uppercase tracking-wide py-2 pl-4">Avg / Order</th>
                    <th className="text-left text-xs font-semibold text-muted uppercase tracking-wide py-2 pl-6 w-40">Share</th>
                  </tr>
                </thead>
                <tbody>
                  {services.map((s, i) => (
                    <tr key={s.plan} className="border-b border-cream last:border-0 hover:bg-cream/50 transition-colors">
                      <td className="py-3 pr-4">
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full flex-shrink-0 ${
                            i === 0 ? "bg-gold" : i === 1 ? "bg-gold/60" : "bg-gold/30"
                          }`} />
                          <span className="font-semibold text-charcoal">{s.plan}</span>
                          {i === 0 && (
                      ...
                      <td className="py-3 px-4 text-right font-bold text-charcoal">{s.count}</td>
                      <td className="py-3 px-4 text-right font-bold text-gold">{fmt(s.revenue)}</td>
                      <td className="py-3 pl-4 text-right text-muted">{fmt(s.avg)}</td>
                ...
                <tfoot>
                  <tr className="bg-cream/50">
                    <td className="py-3 pr-4 font-bold text-charcoal text-sm">Total</td>
                    <td className="py-3 px-4 text-right font-bold text-charcoal">{services.reduce((s, r) => s + r.count, 0)}</td>
                    <td className="py-3 px-4 text-right font-bold text-gold">{fmt(services.reduce((s, r) => s + r.revenue, 0))}</td>
                    <td className="py-3 pl-4 text-right text-muted">—</td>
                    <td className="py-3 pl-6" />
                  </tr>
                </tfoot>
              </table>
            ...
            <div className="sm:hidden space-y-3">
              {services.map((s, i) => (
                <div key={s.plan} className="bg-cream rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-semibold text-charcoal text-sm">{s.plan}</span>
                    {i === 0 && <span className="text-xs bg-gold/10 text-gold font-semibold px-2 py-0.5 rounded-full">Best Seller</span>}
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                      <p className="text-muted">Orders</p>
                      <p className="font-bold text-charcoal">{s.count}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-muted">Avg/Order</p>
                      <p className="font-bold text-charcoal">{fmt(s.avg)}</p>
                    </div>
                  </div>
                  <div className="mt-2">
      ...
      <div className="bg-white rounded-2xl border border-gold-light/50 p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h3 className="font-semibold text-charcoal">Quarterly Highlights — {year}</h3>
        </div>
        <div className="grid sm:grid-cols-4 gap-4">
          {["Q1","Q2","Q3","Q4"].map((q, i) => {
            ...
            return (
              ...
              >
                <p className="text-xs text-muted font-medium uppercase tracking-wide mb-1">{q}</p>
                <p className="font-serif text-2xl font-bold text-gold">{fmt(row?.revenue || 0)}</p>
                <p className="text-muted text-xs mt-0.5">{row?.orders || 0} orders</p>
